...
useEffect(()=>{
  ...
  className='w-11/12 md:w-56 pt-6 md:bg-gradient-to-r from-purple-200 via-indigo-100 to-sky-200 md:block left-5 md:left-0 right-5 md:right-0 relative md:fixed top-0 md:px-1 pb-44'>
    ...
    <div className={`${menuOpen ? 'block': 'hidden'} md:hidden flex pt-2`}>
      <ul className='sm:flex text-2xl font-semibold flex-col w-full space-y-1 items-center' onClick={()=>setMenuOpen(false)}>
      {
            sections.map((section,id)=>{
                return <li key={id} className='flex flex-row group border-2 border-sky-300 w-full items-center justify-center'>
                    <a href={`#${section.id}`} className='text-xl uppercase'>
                        <div className='flex py-2 pr-2 rounded-r-full'>
                        {/* <div className={`${activeSection === section.id ? 'border-t-2 border-violet-700 sm:w-20':'border-t-2 border-sky-900'} w-10 mt-4 transition-all group-hover:w-20 group-hover:border-violet-700`}></div> */}
